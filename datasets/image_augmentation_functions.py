...
def reshape_and_pad(in_tensor, zoom_value, mode, pad_value=0.):
	...
	padding = [[0, 0],
		...
		[0, 0]]
	#pad the image
	img = tf.pad(img, paddings = padding, mode = mode, constant_values=pad_value)
...
def zoom_image_object(in_tensor, zoom_range, padding_mode = 'CONSTANT', pad_value=0.):
	""" Zoom image objects 
	...
	img = tf.case([(tf.greater(zoom_value, 1.0), lambda : crop_and_reshape(in_tensor, zoom_value))], 
					default=lambda : reshape_and_pad(in_tensor, zoom_value, padding_mode, pad_value))
	return img
...
def random_brightness(in_tensor, max_offset):
	...
	def aug_one_color_channel(in_tensor, max_offset):
		...
		zero_array2 = tf.constant(0.0, shape = shape[:3] + [1], dtype = tf.float32)

		channel = tf.random_uniform([], minval = 0, maxval = 3, dtype = tf.int32)
		offset = tf.case([(tf.equal(channel, 0), lambda: tf.concat([offset_array, zero_array, zero_array2], 3)),
			(tf.equal(channel, 1), lambda: tf.concat([zero_array, offset_array, zero_array2], 3))],
			default = lambda: tf.concat([zero_array, zero_array2, offset_array], 3))
